 ...
 * 
 */
public class VirialCH4AlkaneEHMix {
	...
	public static void main(String[] args) {
        ...
        // Alkane parameter
        double sigmaHb = 3.31; // H in alkane
        double sigmaCH2 = 3.65;//C in CH2(alkane)
        double sigmaCH3 = 3.30;//C in CH3(alkane)
        double sigmaCH2Hb = 0.5 * (sigmaHb + sigmaCH2);
        double sigmaCH3Hb = 0.5 * (sigmaHb + sigmaCH3);
        double sigmaCH2CH3 = 0.5 * (sigmaCH2 + sigmaCH3);
        double sigmaHSRef = sigmaCH3 + 0.5*nSpheres;
        boolean[] flex = new boolean[]{false, nPoints > 2};//CH4:always rigid;alkane depends on chain length and Bn order
        if (nSpheres==1){
        	throw new RuntimeException("The 2nd species cannot be CH4!");
        }
        ...
        if ( (nTypes[0]+nTypes[1])!= nPoints ){
        		System.out.println(nTypes[0]);
        		System.out.println(nTypes[1]);
        ...
        System.out.println("CH4(TraPPE-EH,rigid)+"+nSpheres+"smer n-alkane(TraPPE-EH) overlap sampling B"+nTypes[0]+""+nTypes[1]+" at T="+temperature+" Kelvin");
        ...
        // ------------- CH4 potential & CH4 mayer function, "a" means H or C is from CH4---------- //
        double sigmaHa = 3.31;//middle point of C-H bond
        double sigmaCa = 3.31;//carbon core
        double sigmaCaHa = 3.31;
        double epsilonHa = Kelvin.UNIT.toSim(15.3);
        double epsilonCa = Kelvin.UNIT.toSim(0.01);
        double epsilonCaHa = Math.sqrt((epsilonHa * epsilonCa));
        P2LennardJones pHaHa = new P2LennardJones(space, sigmaHa, epsilonHa);
        P2LennardJones pCaCa = new P2LennardJones(space, sigmaCa, epsilonCa);
        P2LennardJones pCaHa = new P2LennardJones(space, sigmaCaHa, epsilonCaHa);
        PotentialGroup pCH4 = new PotentialGroup(2);
        MayerGeneral fCH4= new MayerGeneral(pCH4);
        // ------------- alkane potential & alkane mayer function, "b" means H or C is from alkane---------- //
        double epsilonHb = Kelvin.UNIT.toSim(15.3);
        double epsilonCH2 = Kelvin.UNIT.toSim(5.0);
        double epsilonCH3 = Kelvin.UNIT.toSim(4.0);
        double epsilonCH2Hb = Math.sqrt(epsilonHb*epsilonCH2);//CH2-H
        double epsilonCH3Hb = Math.sqrt(epsilonHb*epsilonCH3);//CH3-H
        double epsilonCH2CH3 = Math.sqrt(epsilonCH2*epsilonCH3);//CH3-CH2
        P2LennardJones pHbHb = new P2LennardJones(space, sigmaHb, epsilonHb);// H-H
        ...
        P2LennardJones pCH2Hb = new P2LennardJones(space, sigmaCH2Hb, epsilonCH2Hb);//H-CH2
        P2LennardJones pCH3Hb = new P2LennardJones(space, sigmaCH3Hb, epsilonCH3Hb);//H-CH3
        ...
        P2LennardJones pHaHb   = new P2LennardJones(space, 0.5*(sigmaHa  + sigmaHb),  Math.sqrt(epsilonHa  * epsilonHb));
        P2LennardJones pHaCH2  = new P2LennardJones(space, 0.5*(sigmaHa  + sigmaCH2), Math.sqrt(epsilonHa  * epsilonCH2));
        P2LennardJones pHaCH3  = new P2LennardJones(space, 0.5*(sigmaHa  + sigmaCH3), Math.sqrt(epsilonHa  * epsilonCH3));
        P2LennardJones pCaHb   = new P2LennardJones(space, 0.5*(sigmaCa  + sigmaHb),  Math.sqrt(epsilonCa  * epsilonHb));
        P2LennardJones pCaCH2  = new P2LennardJones(space, 0.5*(sigmaCa  + sigmaCH2), Math.sqrt(epsilonCa  * epsilonCH2));
        P2LennardJones pCaCH3  = new P2LennardJones(space, 0.5*(sigmaCa  + sigmaCH3), Math.sqrt(epsilonCa  * epsilonCH3));

        PotentialGroup pCH4AlkaneEH = new PotentialGroup(2);
        MayerGeneral fCH4AlkaneEH= new MayerGeneral(pCH4AlkaneEH);
        ...
        MayerFunction[][] f = new MayerFunction[][]{{fCH4,fCH4AlkaneEH},{fCH4AlkaneEH,fAlkaneEH}};
        ...
        String[] targetDiagramStrings = new String[0]; // add an array of strings///////////////////////////
        ...
        SpeciesAlkaneEH speciesAlkaneEH = new SpeciesAlkaneEH(space, nSpheres); // alkane-TraPPE-EH
        ...
        final SimulationVirialOverlap2 sim = new SimulationVirialOverlap2(space,new ISpecies[]{speciesCH4,speciesAlkaneEH}, 
        ...
        if (nSpheres > 4) { 
            ...
            pIntra.addPotential(pCH3Hb,new ApiIndexList(pairsCH));// H(of CH3)-C(of CH3) potential
            pIntra.addPotential(pHbHb,new ApiIndexList(pairsHH));// H(of CH3)-H(of CH3) potential       
        ...
        if (nSpheres > 5) {
            ...
            pIntra.addPotential(pCH2Hb,new ApiIndexList(pairsH_CH2_1));
            pIntra.addPotential(pCH2Hb,new ApiIndexList(pairsH_CH2_2));
            pIntra.addPotential(pCH2Hb,new ApiIndexList(pairsH_CH2_3));
            pIntra.addPotential(pCH3Hb,new ApiIndexList(pairsH_CH3_1));
            pIntra.addPotential(pCH3Hb,new ApiIndexList(pairsH_CH3_2));
            pIntra.addPotential(pHbHb,new ApiIndexList(pairsH1H1));
            pIntra.addPotential(pHbHb,new ApiIndexList(pairsH1H2));
            pIntra.addPotential(pHbHb,new ApiIndexList(pairsH2H1));
            pIntra.addPotential(pHbHb,new ApiIndexList(pairsH2H2));
            pIntra.addPotential(pHbHb,new ApiIndexList(pairsH3H1));
            pIntra.addPotential(pHbHb,new ApiIndexList(pairsH3H2));
        ...
        if (true) {
              ...
              colorScheme.setColor(speciesCH4.getAtomType(0), Color.blue);
              colorScheme.setColor(speciesCH4.getAtomType(1), Color.red);
