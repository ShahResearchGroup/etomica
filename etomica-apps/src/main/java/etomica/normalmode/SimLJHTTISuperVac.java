...
import etomica.action.ActionMinimizeEnergy;
...
public class SimLJHTTISuperVac extends Simulation {
    ...
    public MCMoveAtom atomMove;
    public PotentialMasterCell potentialMaster;
    public Potential2SoftSpherical potential;
    public SpeciesSpheresMono species;
    public SimLJHTTISuperVac(Space _space, int numAtoms, double density, double temperature, double rc, boolean ss, int[] seeds, boolean doVac) {
        super(_space);
        if (seeds != null) {
        ...
        potentialMaster = new PotentialMasterCell(this, space);
        ...
        box.setNMolecules(species, numAtoms-(doVac?1:0));

        integrator = new IntegratorMC(potentialMaster, getRandom(), temperature, box);
        MeterPotentialEnergy meterPE = new MeterPotentialEnergy(potentialMaster);
        meterPE.setBox(box);
        atomMove = new MCMoveAtom(random, potentialMaster, space);
...
//        ((MCMoveStepTracker)atomMove.getTracker()).setNoisyAdjustment(true);
        ...
        CoordinateDefinitionLeaf coordinateDefinition1 = new CoordinateDefinitionLeaf(box, primitive, basis, space);
        coordinateDefinition1.initializeCoordinates(new int[]{1,1,1});
        ...
        if (doVac) {
            ActionMinimizeEnergy minimizer = new ActionMinimizeEnergy(space, 1000, potentialMaster, box);
            minimizer.actionPerformed();
        }

        coordinateDefinition = new CoordinateDefinitionLeafNull(box, space);
        coordinateDefinition.initializeCoordinates(null);
    }
...
//            params.bpharm = new double[]{0};
        ...
        final SimLJHTTISuperVac sim = new SimLJHTTISuperVac(Space.getInstance(3), numAtoms, density, temperature, rc*Math.pow(density, -1.0/3.0), ss, seeds,doVac);
        if (seeds == null) {
        ...
        if (false) {
            ...
            P2SoftSphericalTruncated potentialT = new P2SoftSphericalTruncatedShifted(sim.getSpace(), potential, cutoffs[nCutoffs-1]-0.01);
...
//            potentialT.setTruncationRadius(0.6*sim.box.getBoundary().getBoxSize().getX(0));
        ...
        MeterSolidDACut meterSolid = new MeterSolidDACut(sim.getSpace(), sim.potentialMaster, sim.coordinateDefinition, cutoffs);
        ...
        for (int i=0; i<uFacCut.length; i++) {
            uFacCut[i] = d.getValue(6*i)*(numAtoms-(doVac?1:0)) - uShort;
        }
...
//        }
        ...
        for(int i=0; i<atoms.size(); i++) {
            ...
            Vector rlat = sim.coordinateDefinition.getLatticePosition(atoms.get(i));
            dr.Ev1Mv2(r, rlat);
            sim.box.getBoundary().nearestImage(dr);
            System.out.println(i+" "+Math.sqrt(dr.squared())+" "+(dr.squared() > 0.5 ? "*******************":""));
        }
    ...
    public static class SimOverlapParam extends ParameterBase {
        ...
        public double rcMax1 = 4;
        public double rcMax0 = 4;
  //      public double[] bpharm = new double[0];   lower or thhis
        public double[] bpharm = new double[]{0};
        public double[] bpharmLJ = new double[0];
        public boolean ss = false;
        public int[] randomSeeds = null;
        public boolean doVac;
    }
}
